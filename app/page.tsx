...
// ── Main page (single route — all navigation is state, no URL changes) ─────────
export default function Page() {
  ...
  useEffect(() => {
    ...
    fetchPromise
      .then(data => {
        ...
        // Backfill winner into yearIndex once the tournament is loaded
  ...
  return (
    <main className="min-h-screen pb-16 overflow-x-hidden">
      ...
      {/* ── Sticky header: wordmark + year selector ── */}
      <div
      ...
      >
        <button
          ...
          onClick={() => {
            const latest = yearIndex?.[0]?.year ?? selectedYear
            if (latest) changeYear(latest as number)
            setTab('brackets')
            setSimKey(0)
